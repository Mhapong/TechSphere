    ...
    return (
        <div className="container mx-auto mt-10 p-5">
            ...
            {ownedCourses.length > 0 ? (
                ...
                <div>
                    <p className="text-lg text-gray-600 mb-4">❌ คุณยังไม่ได้ลงทะเบียนคอร์สใด ๆ</p>
                    <h2 className="text-xl font-bold mb-4">🔥 คอร์สแนะนำยอดนิยม</h2>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
                        {courseData && courseData.length > 0 ? (
                            courseData.slice(0, 3).map((course) => (
                                <Card key={course.id} sx={{ p: 3, boxShadow: 3, mb: 3 }}>
                                    <h2 className="text-xl font-semibold">
                                        {course.attributes?.Name || "ไม่พบชื่อคอร์ส"}
                                    </h2>
                                    <p>{course.attributes?.Description || "ไม่มีคำอธิบาย"}</p>
                                </Card>
                            ))
                        ) : (
                            <p>⏳ กำลังโหลดคอร์สแนะนำ...</p>
                        )}
                    </div>
                </div>
            )}
